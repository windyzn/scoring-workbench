...
const CANCER_SYSTEMS = [
    {
        id: "cancer_inflammation", name: "Thromboinflammation", processes: {
            "Thromboinflammation": ["C-reactive protein", "Alpha-1-acid glycoprotein 1", "Alpha-1-antichymotrypsin", "Ceruloplasmin", "Haptoglobin", "Serum amyloid A-1 protein", "Histamine", "Asymmetric dimethylarginine", "Phenylalanine", "Fibrinogen alpha chain", "Fibrinogen beta chain", "Fibrinogen gamma chain", "Coagulation factor XIII A chain", "Coagulation factor XI", "Coagulation factor XIII B chain", "Antithrombin-III", "Ficolin-2"],
        },
        levels: {
            "Thromboinflammation::C-reactive protein": "high",
            "Thromboinflammation::Alpha-1-acid glycoprotein 1": "high",
            "Thromboinflammation::Alpha-1-antichymotrypsin": "high",
            "Thromboinflammation::Ceruloplasmin": "high",
            "Thromboinflammation::Haptoglobin": "high",
            "Thromboinflammation::Serum amyloid A-1 protein": "high",
            "Thromboinflammation::Histamine": "both",
            "Thromboinflammation::Asymmetric dimethylarginine": "high",
            "Thromboinflammation::Phenylalanine": "high",
            "Thromboinflammation::Fibrinogen alpha chain": "high",
            "Thromboinflammation::Fibrinogen beta chain": "high",
            "Thromboinflammation::Fibrinogen gamma chain": "high",
            "Thromboinflammation::Coagulation factor XIII A chain": "both",
            "Thromboinflammation::Coagulation factor XI": "both",
            "Thromboinflammation::Coagulation factor XIII B chain": "both",
            "Thromboinflammation::Antithrombin-III": "low",
            "Thromboinflammation::Ficolin-2": "high",
        },
    },
    {
        ...
        levels: {
            ...
            "Cell Proliferation::Tyrosine": "both",
    ...
    },
    {
        ...
        levels: {
            "Metabolic Dysfunction::Glutamine": "both",
            "Metabolic Dysfunction::Glutamic acid": "both",
            "Metabolic Dysfunction::Glycine": "high",
            "Metabolic Dysfunction::Serine": "high",
            "Metabolic Dysfunction::Proline": "both",
            "Metabolic Dysfunction::Isoleucine": "both",
            "Metabolic Dysfunction::Valine": "both",
            "Metabolic Dysfunction::Alanine": "both",
            "Metabolic Dysfunction::Threonine": "both",
            ...
            "Metabolic Dysfunction::Choline": "both",
            "Metabolic Dysfunction::Betaine": "high",
            "Metabolic Dysfunction::Sarcosine": "high",
            "Metabolic Dysfunction::Histidine": "both",
        },
    },
    {
        id: "cancer_immune_system_evasion", name: "Immune System Evasion", processes: {
            "Immune System Evasion": ["Tryptophan", "Kynurenine", "Arginine", "Complement factor I", "Complement component C9", "Complement factor B", "CD5 antigen-like", "Ficolin-3", "Mannan-binding lectin serine protease 2", "Complement C1q subcomponent subunit B", "C4b-binding protein alpha chain", "Transthyretin", "Serotransferrin", "Vitamin D-binding protein", "L-selectin", "Plasma protease C1 inhibitor"],
        },
        levels: {
            "Immune System Evasion::Tryptophan": "both",
            "Immune System Evasion::Kynurenine": "both",
            "Immune System Evasion::Arginine": "both",
            ...
            "Immune System Evasion::Mannan-binding lectin serine protease 2": "both",
            ...
            "Immune System Evasion::L-selectin": "high",
            "Immune System Evasion::Plasma protease C1 inhibitor": "both",
        },
    },
    {
        id: "cancer_matrix_remodelling", name: "Matrix Remodelling", processes: {
            "Matrix Remodelling": ["Fibronectin", "Vitronectin", "Galectin-3-binding protein", "Alpha-2-macroglobulin", "Alpha-1B-glycoprotein", "Kininogen-1", "Protein AMBP", "Phospholipid transfer protein"],
        },
        levels: {
            "Matrix Remodelling::Fibronectin": "both",
            "Matrix Remodelling::Vitronectin": "both",
            ...
            "Matrix Remodelling::Phospholipid transfer protein": "both",
        },
    },
    {
        id: "cancer_metastasis", name: "Metastasis", processes: {
            "Metastasis": ["Intercellular adhesion molecule 1", "Gamma-aminobutyric acid", "Apolipoprotein D", "Apolipoprotein M", "Plasma serine protease inhibitor", "Beta-2-glycoprotein 1", "Retinol-binding protein 4", "Apolipoprotein E", "Apolipoprotein C-I", "Complement C1r subcomponent", "Hemopexin", "Alpha-2-HS-glycoprotein", "Apolipoprotein A-I", "Gelsolin", "Adipocyte plasma membrane-associated protein"],
        },
        levels: {
            "Metastasis::Intercellular adhesion molecule 1": "both",
            ...
            "Metastasis::Gelsolin": "both",
            "Metastasis::Adipocyte plasma membrane-associated protein": "high",
        },
    },
];
...
function scoreBM(v, lo, hi, cutoff = 0.5, gp = 0.05, curve = "linear", level = "both") {
    const rng = hi - lo; if (rng <= 0) return null;
    const gL = lo + gp * rng, gH = hi - gp * rng;
    if (v >= gL && v <= gH) return 100;
    if (level === "high" && v < gL) return 100;  // only high impacts — being low has no association
    if (level === "low"  && v > gH) return 100;  // only low impacts — being high has no association
...
// Part-to-whole visual for a tier breakdown — a horizontal stacked bar, one segment
// per tier, width proportional to its share. Segments carry the same status colors
// as everywhere else in the app; the 2px gaps are the surface color showing through
// a flex gap (no border needed to separate touching segments). Near-zero segments
// are dropped so they don't eat a gap's worth of space for nothing.
function TierBar({ pct, tierMeta, width = 100, height = 7 }) {
    const segments = tierMeta.filter(t => pct[t.label] > 0.5);
    if (!segments.length) return <div style={{ width, height }} />;
    return (
        <div style={{ display: "flex", gap: 2, width, height, borderRadius: height / 2, overflow: "hidden" }}>
            {segments.map(t => (
                <div key={t.label} title={`${t.label}: ${Math.round(pct[t.label])}%`}
                    style={{ width: `${pct[t.label]}%`, background: t.color }} />
            ))}
        </div>
    );
}
...
function RangeBar({ value, refLow, refHigh, greenPct = 0.05 }) {
    ...
    return (
        <div style={{ position: "relative", height: 28, marginTop: 6 }}>
            ...
            <div style={{
                position: "absolute", top: 6, left: `${vp}%`, transform: "translateX(-50%)",
            ...
            <div style={{
                position: "absolute", top: 21, left: `${vp}%`, transform: "translateX(-50%)",
                fontSize: 9, color: dotCol, fontWeight: 700, whiteSpace: "nowrap"
            }}>{status}</div>
        </div>
    );
}

function ScoringCurve({ refLow, refHigh, value, cutoff, greenPct, curve, level = "both" }) {
    ...
    const pts = Array.from({ length: 120 }, (_, i) => { const x = xMin + (i / 119) * (xMax - xMin); return [x, scoreBM(x, refLow, refHigh, cutoff, greenPct, curve, level)]; });
    const pathD = pts.map(([x, y], i) => { const [sx, sy] = sv(x, y); return `${i === 0 ? "M" : "L"}${sx.toFixed(1)},${sy.toFixed(1)}`; }).join(" ");
    const cs = scoreBM(value, refLow, refHigh, cutoff, greenPct, curve, level);
...
export default function App() {
    ...
    const [uploadModalStage, setUploadModalStage] = useState("choose"); // "choose"|"weight-name"|"success"
    ...
    useEffect(() => {
        ...
        const STEP_TARGETS = [
            ...
            "second-profile-card", // 13 clean up
            null,                  // 14 done
        ];
    ...
    const isDefaultDirty = activeProfileId === "default" && (() => {
        ...
        const isNonDefaultBio = Object.values(bioWeights).some(e =>
            (e?.weight ?? 1) !== 1 || (e?.color ?? "red") !== "red" || (e?.level ?? "both") !== "both");
    ...
    const handleWeightFile = useCallback((file, profileName, onSuccess, onError) => {
        if (!file) return;
        const r = new FileReader();
        r.onload = e => {
            try {
                ...
                rows.forEach(row => {
                    ...
                    const level = (row["level"] ?? "both").trim().toLowerCase();
    ...
    const adjustmentCount = useMemo(() => {
        const bioAdj = Object.entries(bioWeights).filter(([, e]) =>
            (e?.weight ?? 1) !== 1 || (e?.color ?? "red") !== "red" || (e?.level ?? "both") !== "both").length;
    ...
    return (
        <div style={{ minHeight: "100vh", background: C.white, fontFamily: T.body, color: C.textPrimary, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{
                ...
                <div data-tutorial="view-toggle" style={{ display: "flex", background: "#1a3e55", borderRadius: 6, padding: 2, marginLeft: 8 }}>
                    {[["client", "Adjust Weighting"], ["aggregate", "Aggregate Statistics"], ["associations", "Associations"]].map(([v, l]) => (
                        <button key={v} onClick={() => {
                            ...
                            fontWeight: activeView === v ? 700 : 400, transition: "all 0.15s"
                        }}>{l}</button>
                    ))}
                </div>
                <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
                    <select data-tutorial="profile-selector" value={activeProfileId} onChange={e => setActiveProfileId(e.target.value)}
                        style={{
                            background: "#1e4560", border: `1px solid #2d607e`, color: C.iceLight,
            ...
            {concWarnModal && (
                <div style={{
                    position: "fixed", inset: 0, background: "rgba(24,55,75,0.6)", zIndex: 600,
                    display: "flex", alignItems: "center", justifyContent: "center"
                }}
                    onClick={() => setConcWarnModal(false)}>
                    <div style={{
                        background: C.surface, borderRadius: 14, width: 420, padding: 0,
            ...
            {profileModal && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(24,55,75,0.55)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setProfileModal(false)}>
                    <div data-tutorial="profile-modal" style={{ background: C.surface, borderRadius: 14, padding: 0, width: 720, maxHeight: "82vh", boxShadow: "0 12px 48px rgba(24,55,75,0.3)", display: "flex", flexDirection: "column", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
                        {/* header */}
                        ...
                        <div style={{ overflowY: "auto", flex: 1, padding: "14px 24px" }}>
                            {profiles.map((p, pi) => {
                                ...
                                return (
                                    <div key={p.id} {...(pi === 1 ? { "data-tutorial": "second-profile-card" } : {})} style={{
                                        border: `1px solid ${isActive ? col : C.border}`, borderRadius: 10, marginBottom: 12,
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: `1px solid ${C.border}` }}>
                                            ...
                                            <button onClick={() => exportProfile(p)}
                                                data-tutorial="export-btn" style={{ fontSize: 11, padding: "4px 10px", border: `1px solid ${C.teal}66`, borderRadius: 6, cursor: "pointer", background: "transparent", color: C.teal }}>⬇ Export</button>
            ...
            {showTutorial && (() => {
                ...
                const STEPS = [
                    ...
                    // 13
                    {
                        ...
                        spotlight: "second-profile-card", advance: "next"
                    },
                    // 14
                    {
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <div style={{ height: 3, background: C.iceLight }}>
                                <div style={{
                                    height: "100%", background: C.teal, transition: "width 0.35s ease",
                            ...
                            <div style={{ padding: "13px 15px 10px" }}>
                                ...
                                {step.cta && !actionComplete && (
                                    <div style={{
                                        background: `${C.teal}14`, border: `1px solid ${C.teal}50`, borderRadius: 8,
                                ...
                                {actionComplete && (
                                    <div style={{
                                        background: `${C.green}18`, border: `1px solid ${C.green}50`, borderRadius: 8,
                            ...
                            <div style={{ padding: "6px 15px 13px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                <div style={{ display: "flex", gap: 3, flexWrap: "wrap", maxWidth: 160 }}>
                                    {STEPS.map((_, i) => (
                                        <span key={i} style={{
                                            width: i === tutorialStep ? 12 : 4, height: 4, borderRadius: 2,
                                ...
                                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                                    ...
                                    {canGoBack && (
                                        <button onClick={() => setTutorialStep(t => t - 1)}
                                            style={{
                                                padding: "5px 12px", border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 11,
            ...
            {defaultDirtyModal && (
                <div style={{
                    position: "fixed", inset: 0, background: "rgba(24,55,75,0.55)", zIndex: 200,
                    display: "flex", alignItems: "center", justifyContent: "center"
                }}
                    onClick={() => setDefaultDirtyModal(false)}>
                    <div style={{
                        background: C.surface, borderRadius: 12, padding: 28, width: 340,
                        boxShadow: "0 8px 32px rgba(24,55,75,0.28)"
                    }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flex: 1, height: "calc(100vh - 50px)", overflow: "hidden" }}>
                ...
                {showCol2 && <div style={{ width: col2Open ? 196 : 28, flexShrink: 0, background: `${C.iceLight}30`, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", transition: "width 0.2s ease", overflow: "hidden" }}>
                    ...
                    <button onClick={() => setCol2Open(o => !o)}
                        title={col2Open ? "Collapse panel" : "Expand panel"}
                        style={{
                        ...
                        {(activeCancerView || isCancerSystem) ? (() => {
                            ...
                            return <>
                                ...
                                {healthSystem.systems.map(sysId => {
                                    ...
                                    const active = systemId === sysId && !activeCancerView && activeView === "client";
                                    return (
                                        <button key={sysId} data-sysid={sysId}
                                            onClick={() => { setSystemId(sysId); setActiveProc(null); setActiveCancerView(false); setActiveView("client"); }}
                                            style={{ display: "block", width: "100%", textAlign: "left", padding: "10px 14px", border: "none", cursor: "pointer", transition: "all 0.12s", background: active ? `${C.teal}22` : "transparent", borderLeft: `3px solid ${active ? C.teal : "transparent"}` }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6 }}>
                                                <span style={{ fontSize: 11, color: active ? C.navy : C.textSecond, fontWeight: active ? 600 : 400, lineHeight: 1.3, flex: 1 }}>{sys.name}</span>
                                                {s != null && <span style={{ fontSize: 11, color: procColour(s), fontWeight: 700, fontFamily: T.mono, flexShrink: 0 }}>{Math.floor(s)}</span>}
                                            </div>
                                            {s == null && <div style={{ fontSize: 9, color: C.textFaint, marginTop: 2, fontStyle: "italic" }}>No data</div>}
                            ...
                            {[...procResults].sort((a, b) => (a.score ?? Infinity) - (b.score ?? Infinity)).map(pr => {
                                ...
                                return (
                                    <button key={pr.process} onClick={() => { setActiveProc(pr.process); setActiveView("client"); }}
                                        style={{ display: "block", width: "100%", textAlign: "left", padding: "11px 16px", border: "none", cursor: "pointer", transition: "all 0.12s", background: active ? `${C.teal}22` : "transparent", borderLeft: `3px solid ${active ? C.teal : "transparent"}` }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", paddingTop: "24vh", paddingBottom: "14vh", height: "100%", gap: 24 }}>
            ...
            <div data-tutorial="upload-dropzone" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, width: "100%", maxWidth: 520 }}>
                <div onClick={() => !isUploading && fileRef.current?.click()}
                    ...
                    style={{
                        border: `1.5px dashed ${isUploading ? C.teal : dragOver ? C.teal : C.iceMid}`, borderRadius: 12, padding: "28px 48px",
                    ...
                    {uploadErr && !isUploading && (
                        <div style={{
                            marginTop: 12, fontSize: 11, color: C.critical, textAlign: "left",
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, width: "100%" }}>
                    {personas.map(p => (
                        <button key={p.id} onClick={() => loadDemo(p.id)}
                            style={{
                                padding: "14px 12px", borderRadius: 10, border: `1.5px solid ${C.teal}44`,
...
function ProcWeightsTab({ system, procResults, procWeights, setProcWeights, sysScore, selProc, setActiveProc, setTab, card }) {
    ...
    return (
        <div>
            ...
            <div style={{
                ...card, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18,
        ...
        <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                        ...
                        {[
                            ...
                            <div key={label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", border: `1px solid ${C.border}`, borderRadius: 6, overflow: "hidden" }}>
                                    ...
                                    <span style={{
                                        fontSize: 12, fontWeight: 700, color: col, fontFamily: T.mono,
                                ...
                                {val !== def && (
                                    <button onClick={() => set(def)}
                                        style={{
                                            fontSize: 10, padding: "2px 6px", borderRadius: 4, border: `1px solid ${C.border}`,
                                            background: "transparent", color: C.textFaint, cursor: "pointer"
                                        }}>↺</button>
                                )}
                            </div>
                        ))}
                        <Tooltip text={"Global multiplier applied to red-zone biomarkers when no manual weight override is active.\n\nOnly red-zone biomarkers contribute to the health area score — green and yellow biomarkers are treated as optimal and do not pull the score down.\n\nRed (default 4×): scales how heavily a red-zone biomarker influences the health area score relative to others.\n\nHigher values give out-of-range markers stronger influence on the health area score."}>
                            <span style={{
                                display: "inline-flex", alignItems: "center", justifyContent: "center",
                                width: 14, height: 14, borderRadius: "50%", background: C.iceMid,
                                color: C.navy, fontSize: 9, fontWeight: 700, cursor: "default", userSelect: "none"
                            }}>?</span>
                        </Tooltip>
                    </div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(330px,1fr))", gap: 18 }}>
                {sortedBiomarkers.map((bm, bmIdx) => {
                    ...
                    return (
                        <div key={bm.name} {...(isFirstVisible ? { "data-tutorial": "first-bio-card" } : {})} style={{
                            ...card, borderLeft: `3px solid ${zoneCol}`, padding: 13,
                                    ...
                                    <div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px dashed ${C.border}` }}
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                                            ...
                                            {isOverridden && (
                                                <button onClick={() => { const next = { ...concOverrides }; delete next[bm.name]; setConcOverrides(next); }}
                                                    style={{
                                                        fontSize: 9, padding: "1px 6px", borderRadius: 4, border: `1px solid ${C.border}`,
                                                        background: "transparent", color: C.textFaint, cursor: "pointer"
                                                    }}>↺ restore</button>
                                            )}
                                        </div>
                                            ...
                                            {zonePresets.map(({ label, col, value: pv }) => {
                                                ...
                                                return (
                                                    <button key={label} onClick={() => setConc(pv)}
                                                        style={{
                                                            flex: 1, minWidth: 0, fontSize: 9, padding: "4px 2px", borderRadius: 4,
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                            ...
                                            <input
                                                ...
                                                style={{
                                                    width: 72, fontSize: 11, padding: "3px 7px", border: `1px solid ${C.fair}66`,
...
function BiomarkerDetailModal({ bm, bioWeights, cutoff, greenPct, curve, onClose, assocSystems }) {
    ...
    return (
        <div style={{
            position: "fixed", inset: 0, background: "rgba(24,55,75,0.55)", zIndex: 500,
            display: "flex", alignItems: "center", justifyContent: "center"
        }}
            onClick={onClose}>
            <div style={{
                background: C.surface, borderRadius: 14, width: 440, maxHeight: "88vh",
                ...
                {/* Header */}
                <div style={{
                    background: C.navy, padding: "14px 18px", borderRadius: "14px 14px 0 0",
                    ...
                    <button onClick={onClose} style={{
                        background: "none", border: "none", color: C.iceMid,
                    ...
                    <div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {memberships.map(({ sys, proc }, i) => (
                                <div key={i} style={{
                                    fontSize: 11, color: C.textSecond, background: C.iceLight,
                            ...
                            <div>
                                ...
                                <div style={{ ...card, padding: 0, overflow: "auto" }}>
                                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                                        <thead>
                                            <tr style={{ background: C.navy }}>
                                                <th style={{ padding: "9px 16px", textAlign: "left", color: C.iceLight, fontSize: 11, fontWeight: 600, width: 180 }}>Score</th>
                                                {aggregateData.map(({ profile }, pi) => {
                                                    const col = PROF_COLORS[pi % PROF_COLORS.length];
                                                    return STAT_COLS.map(stat => <th key={`${profile.id}-${stat}`} style={{ padding: "9px 10px", textAlign: "center", color: C.iceLight, fontSize: 10, fontWeight: 600, whiteSpace: "nowrap", borderLeft: stat === "Mean" ? `2px solid ${col}40` : undefined }}>{stat}</th>);
                                                })}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {/* Cancer domain row */}
                                            {(() => {
                                                const allStats = aggregateData.map(pd => cancerDomainStats(pd));
                    ...
                    <div>
                        <div data-tutorial="client-scores-table" style={{ marginBottom: 32 }}>
                            ...
                            {(() => {
                                ...
                                return (
                                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                                        <button onClick={downloadSysCSV} style={{
                                            fontSize: 10, padding: "3px 10px", borderRadius: 5, cursor: "pointer",
                                            border: `1px solid ${C.teal}55`, background: "transparent", color: C.teal, fontWeight: 600,
                                        }}>⬇ CSV</button>
                    ...
                    return (
                        <div>
                            ...
                            <div data-tutorial="client-scores-table" style={{ marginBottom: 32 }}>
                                ...
                                <button onClick={downloadProcCSV} style={{
                                    fontSize: 10, padding: "3px 10px", borderRadius: 5, cursor: "pointer",
                                    border: `1px solid ${C.teal}55`, background: "transparent", color: C.teal, fontWeight: 600,
                                }}>⬇ CSV</button>
                                                    ...
                                                    {rows.map((row, ri) => {
                                                        ...
                                                        return (
                                                            <tr key={row.pid} style={{ borderTop: `1px solid ${C.border}` }}>
                                                                ...
                                                                {allProcs.map(procName => {
                                                                    const score = row.systems.flatMap(s => (s.procs ?? []).filter(p => p.name === procName).map(p => p.score))[0] ?? null;
                                                                    const baseScore = baseRow?.systems.flatMap(s => (s.procs ?? []).filter(p => p.name === procName).map(p => p.score))[0] ?? null;
                                                                    const d = score != null && baseScore != null ? score - baseScore : null;
                                                                    return (
                                                                        <td key={procName} style={{ padding: "5px 4px", textAlign: "center", background: overviewBg(score) }}>
                                                                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                                                                                ...
                                                                                {d != null && Math.abs(d) > 0.05 && <span style={{ fontSize: 9, fontFamily: T.mono, fontWeight: 700, lineHeight: 1, color: d > 0 ? C.teal : C.critical }}>{d > 0 ? "▲" : "▼"}{Math.abs(d).toFixed(1)}</span>}
                                                                            </div>
                                                                        </td>
                                                                    );
                                                                })}
                                ...
                                <div style={{ ...card, padding: 0, overflow: "auto" }}>
                                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                                        <thead>
                                            {isComparing && (
                                                <tr style={{ background: `${C.navy}08`, borderBottom: `1px solid ${C.border}` }}>
                                                    <th style={{ padding: "8px 16px", textAlign: "left", width: 200 }} />
                                                    {aggregateData.map(({ profile }, pi) => {
                                                        const col = PROF_COLORS[pi % PROF_COLORS.length];
                                                        return <th key={profile.id} colSpan={pi > 0 ? STAT_COLS.length + 1 : STAT_COLS.length} style={{ padding: "8px 10px", textAlign: "center", borderLeft: `2px solid ${col}`, color: col, fontSize: 11, fontWeight: 700 }}><span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: col, display: "inline-block" }} />{profile.name}</span></th>;
                                                    })}
                                        ...
                                        <tbody>
                                            {procNames.map((procName, pi) => {
                                                const allStats = aggregateData.map(pd => procStatsForProfile(pd, procName));
                                                const baseStats = allStats[0];
                                                const procC = overviewColour(baseStats?.mean);
                                                return (
                                                    <tr key={procName} style={{ borderTop: `1px solid ${C.border}`, background: pi % 2 === 0 ? "transparent" : `${C.iceLight}20` }}>
                                                        ...
                                                        {aggregateData.map(({ profile }, pii) => {
                                                            ...
                                                            const deltaMean = pii > 0 && st && baseStats ? st.mean - baseStats.mean : null;
                                                            return [
            ...
            <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <input type="number" min={1} max={yellowCutoff - 1} value={redCutoff}
                        ...
                        style={{
                            width: 52, fontSize: 11, padding: "3px 6px", border: `1px solid ${C.border}`,
...
function FlowchartTab({ flowSysId, setFlowSysId, flowCancerHealthSystemId, setFlowCancerHealthSystemId, isCancerFlow, setIsCancerFlow, bioWeights, procWeights, cancerSysWeights, cancerHealthSystemWeights, card, assocSystems, sysGroups }) {
    ...
    const bioHeights = allBiomarkers.map(bmName => {
        ...
        const isModified = (be.weight ?? 1) !== 1 || (be.color ?? "red") !== "red" || (be.level ?? "both") !== "both";
                        ...
                        <div key={label}>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                {groupId === "cancer" ? CANCER_HEALTH_SYSTEMS.map(healthSystem => {
                                    ...
                                    <button key={s.id} onClick={() => { setFlowSysId(s.id); setIsCancerFlow(false); }}
                                        style={{
                                            padding: "5px 12px", fontSize: 11, borderRadius: 6, cursor: "pointer",
                                            border: `1px solid ${!isCancerFlow && flowSysId === s.id ? C.steel : C.border}`,
                                            background: !isCancerFlow && flowSysId === s.id ? `${C.steel}18` : "transparent",
                                            color: !isCancerFlow && flowSysId === s.id ? C.steel : C.textMuted, fontWeight: !isCancerFlow && flowSysId === s.id ? 700 : 400
                                        }}>
                                        {s.name}
                                    </button>
                                ))}
            ...
            <div style={{ overflowX: "auto" }}>
                <svg ref={svgRef} width={totalW} height={totalH} style={{ display: "block", fontFamily: T.body, overflow: "visible" }}
                    ...
                    {allBiomarkers.map((bmName, bi) => {
                        ...
                        const dir = be.level ?? "both";
                        const isModified = w !== 1 || col !== "red" || dir !== "both";
                        const cy = bCY(bi);
                        ...
                        return (
                            <g key={bmName}
                                ...
                                {isModified && (() => {
                                    ...
                                    if (dir !== "both") { badges.push(badge(dir === "high" ? "↑" : "↓", C.atRisk, bx, by)); }
                                    return badges;
                                })()}
                            </g>
                        );
                        ...
                        {filtered.map((row, i) => (
                            <tr key={`${row.systemId}::${row.process}::${row.biomarker}`}
                                ...
                                <td style={{ padding: "7px 14px", color: row.group === "cancer" ? C.textMuted : isTwoLevelGroup(row.group) ? C.border : C.textMuted, fontStyle: row.group === "cancer" ? "normal" : isTwoLevelGroup(row.group) ? "italic" : "normal" }}>
                                    {row.group === "cancer" ? row.systemName : isTwoLevelGroup(row.group) ? "—" : row.process}
                                </td>
